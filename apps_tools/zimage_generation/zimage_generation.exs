...
# Check device and memory requirements
if torch.cuda.is_available():
    # Check if GPU has enough memory (~6GB for Z-Image-Turbo)
    gpu_memory_gb = torch.cuda.get_device_properties(0).total_memory / (1024**3)
    if gpu_memory_gb < 6.0:  # Minimum requirement
        print(f"[WARN] GPU has {gpu_memory_gb:.1f}GB memory, using CPU instead (minimum 6GB recommended)")
        device = "cpu"
        dtype = torch.float32
    else:
        print(f"[OK] GPU detected with {gpu_memory_gb:.1f}GB memory, using CUDA")
        device = "cuda"
        dtype = torch.bfloat16
else:
    print("[INFO] No GPU detected, using CPU (might be slow)")
    device = "cpu"
    dtype = torch.float32
...
if device == "cuda":
    ...
    # Note: Requires Triton package and CUDA compatibility. May cause stream issues.
    try:
        # Check if Triton is available before compiling
        import triton
        # Disable torch.compile to avoid CUDA stream capturing issues
